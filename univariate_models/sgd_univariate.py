"""
This script performs direct 1-step (default) SGD Regression autoregressive forecasts with an expanding-window re-training approach and hv-block CV. Additionally, SHAP values are computed for a subsequent explainability analysis.
Fur further methodological details and motivation behind methodological decisions, please refer to the thesis.

Output:
- 1_sgd_univariate_forecasts.csv
- 1_sgd_univariate_shap_values.csv
- 1_sgd_univariate_best_params.csv
...
Usage in CLI:
    python univariate_models/sgd_univariate.py --input data_intermediate/transformed_univariate_1.csv --datecol sasdate --start-train-months 100
"""
...
def compute_shap_and_forecasts(
    ...
    for e in [0.1, 1, 1.1, 1.35, 1.5, 1.75, 2] # tuning the robustness threshold
    ...
    for origin_idx in range(initial_train_months, n - horizon + 1):
        ...
        if (origin_idx - initial_train_months) % cv_interval == 0:
            tuned_params = hv_block_cv(SGDRegressor, X_train, y_train, param_grid, h=h, v=v, n_splits=1, winsorize="huber")
...
def main(args):
    ...
    shap_out = args.output or "results/shap/1_sgd_univariate_shap_values.csv"
    shap_df.to_csv(shap_out, index=False)
    print(f"\nSaved SHAP values to {shap_out}")

    forecast_out = "results/forecast/1_sgd_univariate_forecasts.csv"
    forecast_df.to_csv(forecast_out, index=False)
    print(f"Saved forecasts to {forecast_out}")

    best_params_out = "results/forecast/1_sgd_univariate_best_params.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Direct SGD Regression AR forecasting with SHAP (1-step only)"
    )
