"""
This script performs direct 1-step (default) Elastic Net multivariate forecasts with an expanding-window re-training approach and hv-block CV. Additionally, SHAP values are computed for a subsequent explainability analysis.
...
Output:
- 1_elasticnet_multivariate_shap_values.csv
- 1_elasticnet_multivariate_forecasts.csv
- 1_elasticnet_multivariate_best_params.csv
...
Usage in CLI:
    python multivariate_models/elasticnet_multivariate.py --input data_intermediate/transformed_multivariate_1.csv --datecol sasdate --nlags 12 --predictor-lags 1 --start-train-months 100 --verbose
"""
...
def main(args):
    ...
    shap_out = args.output or "results/shap/1_elasticnet_multivariate_shap_values.csv"
    shap_df.to_csv(shap_out, index=False)
    print(f"\nSaved SHAP values to {shap_out}")

    forecast_out = "results/forecast/1_elasticnet_multivariate_forecasts.csv"
    forecast_df.to_csv(forecast_out, index=False)
    print(f"Saved forecasts to {forecast_out}")
          
    best_params_out = "results/forecast/1_elasticnet_multivariate_best_params.csv"
