...
Output:
- 1_sgd_multivariate_shap_values.csv
- 1_sgd_multivariate_forecasts.csv
- 1_sgd_multivariate_best_params.csv
...
Usage in CLI:
    python multivariate_models/sgd_multivariate.py --input data_intermediate/transformed_multivariate_1.csv --datecol sasdate --nlags 12 --predictor-lags 1 --start-train-months 100 --verbose
"""
...
def compute_shap_and_forecasts(
    ...
    for origin_idx in range(initial_train_months, n - horizon + 1):
        ...
        if (origin_idx - initial_train_months) % cv_interval == 0:
            tuned_params = hv_block_cv(SGDRegressor, X_train, y_train, param_grid,n_splits=1, h=h, v=v, winsorize = "huber")
...
def main(args):
    ...
    shap_out = args.output or "results/shap/1_sgd_multivariate_shap_values.csv"

    shap_df.to_csv(shap_out, index=False)
    print(f"\nSaved SHAP values to {shap_out}")

    forecast_out = "results/forecast/1_sgd_multivariate_forecasts.csv"
    forecast_df.to_csv(forecast_out, index=False)
    print(f"Saved forecasts to {forecast_out}")
          
    best_params_out = "results/forecast/1_sgd_multivariate_best_params.csv"
