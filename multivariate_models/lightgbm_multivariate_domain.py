"""
This script performs direct 1-step (default) LightGBM multivariate forecasts with an expanding-window re-training approach and hv-block CV. Additionally, SHAP values are computed for a subsequent explainability analysis.
...
Output:
- 1_lightgbm_multivariate_domain_shap_values.csv
- 1_lightgbm_multivariate_domain_forecasts.csv
- 1_lightgbm_multivariate_domain_best_params.csv
...
Usage in CLI:
    python multivariate_models/lightgbm_multivariate_domain.py --input data_intermediate/transformed_multivariate_domain_1.csv --datecol sasdate --nlags 12 --predictor-lags 1 --start-train-months 100 --verbose
"""
...
def compute_shap_and_forecasts(
    ...
    for origin_idx in range(initial_train_months, n - horizon + 1):
        ...
        if (origin_idx - initial_train_months) % cv_interval == 0:
            tuned_params = hv_block_cv(LGBMRegressor, X_train, y_train, param_grid,n_splits=1, h=h, v=v, winsorize = True)
        ...
        shap_vals = explainer.shap_values(scaler.transform(x_live), check_additivity = False)
...
def main(args):
    ...
    shap_out = args.output or "results/shap/1_lightgbm_multivariate_domain_shap_values.csv"
    shap_df.to_csv(shap_out, index=False)
    print(f"\nSaved SHAP values to {shap_out}")

    forecast_out = "results/forecast/1_lightgbm_multivariate_domain_forecasts.csv"
    forecast_df.to_csv(forecast_out, index=False)
    print(f"Saved forecasts to {forecast_out}")
          
    best_params_out = "results/forecast/1_lightgbm_multivariate_domain_best_params.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Direct LGBM multivariate forecasting with SHAP (1-step)"
    )
